...
class Debby {
    ...
    function Debby($dbpath = "", $username = "", $password = "", $dbtype = "sqlite", $outputdateformat = "YYYY-mm-dd", $debug = false) { //possible options are dd/mm/YYYY dd-mm-YYYY dd.mm.YYYY mm/dd/YYYY ... YYYY-mm-dd etc ...
        $this->debug = $debug;
                ...
                foreach ($_FILES as $name => $value) {
                    if ($value["tmp_name"] != "") {
                        if (substr($name, 0, $prefixlen) == $fieldprefix) {
                            ...
                            $this->setBlob($tablename, strtoupper(substr($name, $prefixlen, strlen($name) - $prefixlen)), file_get_contents($value["tmp_name"]), $filter = "$primarykey = '" . $_REQUEST[$requestvar] . "'");
                        }
                    }
                }
            }
            ...
            if ($_FILES) {
                foreach ($_FILES as $name => $value) {
                    if ($value["tmp_name"] != "") {
                        if (substr($name, 0, $prefixlen) == $fieldprefix) {
                            //upload the file correctly into a blob field
                            $this->setBlob($tablename, strtoupper(substr($name, $prefixlen, strlen($name) - $prefixlen)), file_get_contents($value["tmp_name"]), $filter = "$primarykey = '" . $index . "'");
                        }
                    }
                }
            }
