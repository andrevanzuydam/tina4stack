...
class Ruth {
    ...
    public static function initRuth($sessionName) {
        ...
        if (session_save_path() === "") {
          session_save_path(getcwd()."/../temp/sessions");
          @mkdir(getcwd()."/../temp/sessions/");
        }
     ...
     */
    public static function addRoute($requestMethod, $routePath, $routeFunction) {
        self::$routes[] = (object) array("requestMethod" => $requestMethod, "routePath" => $routePath, "routeFunction" => $routeFunction);
        return true;
    }
    ...
    public static function getAuthorization ($routePath="") {
       ...
       if (array_key_exists ($roleName, self::$ROLES)) {
          ...
          if (self::$DEBUG) {
            self::Message(__LINE__.": Allowed ".print_r(self::$ROLES[$roleName], 1));
          } 
